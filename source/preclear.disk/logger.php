...
foreach ($_POST as $key => $value) {
  switch ($key) {
    ...
    case 'done_button':   $done_button = $value; break;
    case 'color_coding':  $color_coding = $value; break;
    case 'file':          $file = $value; break;
    case 'file_position': $file_position = $value; break;
    case 'file_lines':    $file_lines = $value; break;
    case 'command':       $command = $value; break;
    case 'socket_name':   $socket_name = $value; break;
    case 'timeout':       $timeout = $value; break;
  }
}

if (!isset($file_lines)) $file_lines = 30;
if (!isset($timeout)) $timeout = 60;
if (!isset($color_coding) && $file) $color_coding = true;

foreach ($argv as $key => $value) {
  switch ($key) {
    case 1: $command     = $value; break;
    case 2: $socket_name = $value; break;
    case 3: $search      = $value; break;
  }
}

if (PHP_SAPI === "cli" && isset($command) && isset($socket_name))            $task = "command_run";
else if ($action == "get_command" && isset($command) && isset($socket_name)) $task = "command_get_output";
else if ( isset($file) && $action == "get_log" )                             $task = "file_get_lines";
else if (isset($file) && is_file($file) && $action == "download")            $task = "file_download";
else if (isset($file) || isset($command))                                    $task = "show_logger_html";
else exit();
...
if ( $task == "command_run" ) {
  ...
  if (is_resource($process)) {
    debug("INITIATING SERVER: ".$socket_name);

    while ($lock) {

      if (@ $conn = stream_socket_accept($socket, 10)) {
        debug("INIT RESPONSE: ".$socket_name);
        $return = "";   
        ...
        fclose($conn);
        debug("END RESPONSE: ".$socket_name);
        $timer = time();
        if ($do_exit) break;
      }
      debug("LOOP: ".$socket_name);

      if (! proc_get_status($process)['running']) {
        $do_exit = true;
      }

      if ($timeout && isTimeout($timer, $timeout)) break;
    }
  }
  debug("KILLING SERVER: ".$socket_name);
  proc_close($process);
  fclose($socket);
  @ unlink($socket_file);
