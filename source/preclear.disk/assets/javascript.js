...
{
  $.post(PreclearURL,{action:'get_statistics'}, function(data)
  {
    if (data)
    {
      info = [];
      keys = Object.keys(data);
      keys.splice(keys.indexOf('file'),1);
      ...
      swal(
      {
        ...
        text: "Send <span style='font-weight:bold;'>anonymous</span> information (using TOR) to the developer, helping on bug fixes, performance tunning and usage statistics that will be open to the community. For detailed information, please visit the <a href='http://lime-technology.com/forum/index.php?topic=39985.0'>support forum topic</a>.<br><br><span style='font-weight:bold;'>Shared Info:</span> "+info+". ",
        type: "warning",
      ...
      function(isConfirm)
      {
        if (isConfirm)
        {
          $.post(PreclearURL,{action:'send_statistics',file:data.file});
        }
        else
        {
          $.post(PreclearURL,{action:'remove_statistics',file:data.file});
        }
        setTimeout("getOpenStatistics()", 1000);
      }); 
    }
  }, "json");
}
